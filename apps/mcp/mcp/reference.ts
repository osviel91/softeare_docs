...
function eventFlowDslText(): string {
  return [
    ...
    "",
    "Before binding, call `list_semantic_messages`, `list_semantic_occurrences`, and `find_semantic_message_candidates`. Determine from evidence whether equal or similar names are the same architectural message; reuse an authoritative identity or create exactly one, then bind each exact occurrence and verify with `get_semantic_message`. A cross-view message binding does not imply a `complementary-view` relationship. Create that relationship only when the Sequence and Event Flow are substantially the same behavior viewed through execution and causality.",
    "",
