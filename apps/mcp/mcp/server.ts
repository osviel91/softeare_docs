...
8. For complementary projections, call list_resource_relationships before authoring and create_resource_relationship when the typed relationship is absent. Prose such as "complements X" in a description is not a replacement for a typed relationship. For semantic tracing, use list_semantic_messages, list_semantic_occurrences, find_semantic_message_candidates and get_semantic_message before mutating; create_semantic_message establishes identity, and bind_semantic_message/unbind_semantic_message edit source through resource revisions. In Sequence source, author evidence-backed architectural messages with \`semantic event|command publish|consume|dispatch Name [messageRef ID]\`; names are candidates only, never identity. Bind an Event Flow event with \`event Name messageRef ID\` only when evidence establishes the same architectural message. Publish, consume and dispatch describe occurrences; event or command belongs to the shared identity. Do not create complementary-view relationships merely because two resources share a message. Preserve unknown origins and consumers rather than inventing them.
