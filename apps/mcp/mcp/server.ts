...
8. For complementary projections, call list_resource_relationships before authoring and create_resource_relationship when the typed relationship is absent. Prose such as "complements X" in a description is not a replacement for a typed relationship. In Sequence source, author evidence-backed architectural messages with \`semantic event|command publish|consume|dispatch Name\`; do not infer semantics from names, participant roles, or brokers. A producer occurrence and a downstream Event Flow consumer are not automatically complementary views or a stable identity match.
