        ...
        messages: [
          userMessage(
            [
              ...
              "2. Classify needs as business flows, real asynchronous event contexts, Notes, or unsupported Conceptual/Database gaps. Sequence and Event Flow are orthogonal projections: a Sequence may include asynchronous interactions to preserve execution order, while a meaningful causal chain should also be considered as Event Flow. Do not mechanically duplicate every Sequence. HTTP requests, synchronous calls, proxy routing, cron, logs/telemetry, and infrastructure topology do not establish an Event Flow; no asynchronous event context is a valid result. Do not force unsupported or synchronous behavior into available types.",
        ...
        messages: [
          userMessage(
            [
              ...
              "- When real asynchronous behavior exists, use Event -> Handler -> Effects -> Resulting Events as an investigation heuristic, not a mandatory graph. The UpOne-style fan-out from one event through multiple handlers, commands, and persistence effects is a candidate for both an ordered Sequence and an additional causal Event Flow. Use explicit `handler`, `handled by`, `causes`, and `effect` lines for known causal facts; put cross-cutting rules in Markdown and report unsupported Conceptual/Database representation gaps rather than converting them automatically.",
              "",
