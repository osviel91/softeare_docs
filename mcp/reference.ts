...
export const SERVER_INSTRUCTIONS = [
  ...
  "Conventions that make the output good:",
   ...
   "- New resources normally have a concise architectural description and only evidence-supported discovery tags. Notes and details are selective: preserve conditions, uncertainty, constraints, rationale, boundaries, transformations, idempotency, delivery, retry, or other facts the diagram cannot carry; never invent them.",
   "- Sequence semantic messages are explicit local occurrences: use `semantic event|command publish|consume|dispatch Name` only when evidence supports it. Kind and operation are independent; do not infer either from suffixes or brokers. An unbound occurrence is valid and `messageRef` is infrastructure identity, not a human authoring requirement.",
    "- A SemanticMessageIdentity is a stable project-scoped architectural identity. `messageRef` binds Sequence occurrences and Event Flow entities to it; names are display data and equal names are candidates only. Search identities, occurrences, and candidates before reusing or creating one; create with name/kind only, receive the generated id, then bind exact occurrences and verify the authoritative trace. Never invent a UUID.",
    "- A message may be traced across resources without making those resources complementary views. Create a complementary-view relationship only when a Sequence and Event Flow are execution/causal projections of substantially the same behavior and both add materially different information.",
   ...
   "- Legacy enrichment is conservative: inspect existing resources, surface unstructured message candidates, unbound structured occurrences, missing metadata, and missing typed relationships; establish evidence before applying a targeted change, preserve unknowns, and validate afterward. Never rewrite a project wholesale.",
...
export function documentationModelText(): string {
  return [
    ...
    "",
     "A business Sequence may include asynchronous event interactions when they are part of the ordered collaboration. If evidence establishes an architectural message, author `semantic event|command publish|consume|dispatch Name` on the following line. Do not infer it from naming, participant roles, or broker presence; ordinary calls stay ordinary. If those interactions expose a meaningful causal chain, consider an additional Event Flow rather than forcing execution and causality into one representation.",
     "The conceptual model comes before syntax: one SemanticMessageIdentity may have many Sequence occurrences and Event Flow entities. Event messages may be published or consumed; command messages may be dispatched or consumed when the evidence supports that role. `messageRef` is optional in authored source and should be added through the binding workflow, not manually generated by a human.",
    "",
    ...
    "",
    "When real asynchronous behavior exists, use `Event -> Handler/consumer -> Effects -> Resulting events` as an investigation heuristic, not a mandatory shape. The absence of an asynchronous event context is a valid result: say `No asynchronous event context was observed` rather than inventing Event Flow coverage. Topology (`publishes`/`consumes`) is not causality. Use explicit `handler`, `handled by`, `causes`, and `effect` lines; never infer handler outputs from service publications. Partial causal knowledge is valid, and unknown provenance is preferable to invented provenance. Commands and events share the generic message-name slot. Use `details: \"\"\" ... \"\"\"` inside event, handler, or effect blocks for authored plain-text context; keep message kind, domain, provenance, initiation, ownership, and causal relationships in structured fields.",
    "",
    ...
    "An Event Flow event or command can bind to the same SemanticMessageIdentity as a Sequence occurrence. Inspect `list_semantic_messages`, `list_semantic_occurrences`, and `find_semantic_message_candidates`; equal names do not prove identity. Reuse an authoritative identity, otherwise create one identity and bind each evidenced occurrence. This traceability does not imply a complementary-view relationship.",
    "",
    "## Notes",
    "",
    ...
    "6. Inspect semantic occurrences and candidates for progressive enrichment: unstructured Sequence messages that appear to describe architectural messages, structured occurrences or Event Flow entities without `messageRef`, matching cross-resource candidates, and failure/retry facts still trapped in prose. These are leads, not authoritative identity.",
    "7. For identity authoring, discover first, select a compatible existing identity or call `create_semantic_message` with `name` and `kind`, receive the generated id, then bind the exact occurrence. Humans and normal agents never manufacture UUIDs.",
    "8. Validate content and inspect diffs/merge analysis before review.",
    "",
...
export function documentingGuideText(): string {
  return [
    ...
    "- One event flow per event-driven subsystem, if the architecture has one.",
     ...
     "- A concise architectural description and useful evidence-backed tags for each new resource. Do not add generic tags merely to fill fields; details are selective and preserve facts the diagram cannot represent.",
    "",
    "## 3. Draw interactions",
    "",
    ...
    "```",
    "",
     "Declare every service you name, so the index can flag an unknown one. Add metadata (`version`, `domain`, `schema`) — it is what a reader needs to consume the event correctly. For commands, declare `kind: command`; never infer kind from a name suffix.",
    "",
    ...
    "- Run `validate_project` and fix every error.",
     ...
     "- For an existing project, call `list_semantic_occurrences` and `find_semantic_message_candidates` before enriching. Bind only after evidence confirms the same architectural message; use `create_semantic_message` and `bind_semantic_message`, then validate. Do not automatically migrate every legacy resource.",
...
export function workflowGuideText(): string {
  return [
    ...
    "- `list_resource_relationships` — discover semantic complementary views by stable id; use this before authoring a second representation.",
    "",
    "## Read",
    "",
    ...
    "- `create_resource_relationship` — connect an existing Sequence and Event Flow as complementary views only when both materially contribute; roles may be `execution` and `causal`.",
    "",
    "## Check",
    "",
