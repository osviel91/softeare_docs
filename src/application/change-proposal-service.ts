...
export interface ChangeProposalService {
  ...
  list(context: ApplicationContext, projectId: string, resourceId: string): Promise<ChangeProposal[]>;
  update(
    context: ApplicationContext,
    id: string,
    input: ChangeProposalChanges & { expectedVersion: number },
  ): Promise<ChangeProposal>;
  open(context: ApplicationContext, id: string, expectedVersion: number): Promise<ChangeProposal>;
  close(context: ApplicationContext, id: string, expectedVersion: number): Promise<ChangeProposal>;
}
...
export function createChangeProposalService(options: {
  ...
  const projectFor = async (context: ApplicationContext, proposalId: string) => {
    const proposal = await options.proposals.get(proposalId);
    if (proposal === null) throw notFound(`No change proposal with id ${proposalId}.`);
    const resource = await findResource(proposal.resourceId);
    await policy.requirePermission(context, resource.projectId, "resource:read");
  ...
  const write = async (context: ApplicationContext, proposalId: string) => {
    ...
    await policy.requirePermission(context, result.resource.projectId, "resource:update");
    if (result.proposal.status === "closed") throw invalid("Closed proposals cannot be changed.");
    return result;
  };
  const finish = (value: ChangeProposal | null, id: string, expectedVersion: number) => {
    if (value) return value;
    throw conflict(`Change proposal ${id} changed since version ${expectedVersion}.`, {
      expectedVersion,
    });
  };
  ...
  return {
    async create(context, projectId, resourceId, input) {
      ...
      if (!base) throw notFound(`No revision ${baseRevision} exists for resource ${resourceId}.`);
      ...
      const proposal: NewChangeProposal = {
        ...
        ...(base.metadata === undefined ? {} : { proposedMetadata: base.metadata }),
        title,
    ...
    async update(context, id, input) {
      ...
      const changes: ChangeProposalChanges = {
        ...
        ...(input.proposedMetadata === undefined ? {} : { proposedMetadata: normalizeResourceMetadata(input.proposedMetadata) }),
        ...(input.title === undefined ? {} : { title: input.title.trim() }),
        ...(input.description === undefined ? {} : { description: input.description }),
      };
      if (changes.title === "") throw invalid("A proposal title is required.");
      return finish(await options.proposals.update(id, input.expectedVersion, changes), id, input.expectedVersion);
    },
    async open(context, id, expectedVersion) {
      const { resource, proposal } = await write(context, id);
      await policy.requirePermission(context, resource.projectId, "resource:update");
      if (proposal.status !== "draft") throw invalid("Only draft proposals can be opened.");
      return finish(await options.proposals.transition(id, expectedVersion, "draft", "open"), id, expectedVersion);
    },
    async close(context, id, expectedVersion) {
      const { resource, proposal } = await write(context, id);
      await policy.requirePermission(context, resource.projectId, "resource:update");
      if (proposal.status !== "draft" && proposal.status !== "open") throw invalid("Only draft or open proposals can be closed.");
      return finish(await options.proposals.transition(id, expectedVersion, proposal.status, "closed"), id, expectedVersion);
    },
  };
}
